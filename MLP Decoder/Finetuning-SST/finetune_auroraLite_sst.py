...
# Create colormap function for SST visualization
def create_sst_colormap(sst_tensor, mask):
    """Convert SST tensor to RGB image with colormap"""
    sst_np = sst_tensor.cpu().numpy()
    sst_masked = np.where(mask.cpu().numpy(), sst_np, np.nan)
    
    # Normalize for colormap (typical SST range: -2 to 35°C)
    vmin, vmax = 270, 305  # Kelvin
    sst_norm = np.clip((sst_masked - vmin) / (vmax - vmin), 0, 1)
    
    # Apply colormap
    cmap = cm.get_cmap('coolwarm')
    rgb = cmap(sst_norm)[:, :, :3]  # Drop alpha channel
    
    # Set land to gray
    rgb[~mask.cpu().numpy()] = [0.5, 0.5, 0.5]
        ...
        for t in range(history, end_index):
            ...
            if sample_count % accumulation_steps == 0:
                ...
                with torch.no_grad():
                    ...
                    for t_val in range(history, min(history + accumulation_steps, val_surf_subset.sizes['valid_time'])):
                        ...
                        target_val = torch.from_numpy(
                            val_surf_subset["sst"].isel(valid_time=t_val).values
                        ).float().unsqueeze(0).to(device)
                        
                        ocean_mask_val = ~torch.isnan(target_val)
                        ...
                        pred_sst_val = preds_val["sst"].squeeze(1)
                        
                        valid_pred_val = pred_sst_val[ocean_mask_val]
                        valid_target_val = target_val[ocean_mask_val]
                        
                        mae_val = F.l1_loss(valid_pred_val, valid_target_val)
                ...
                if global_step % 100 == 0:
                    pred_colored = create_sst_colormap(pred_sst[0], ocean_mask[0])
                    target_colored = create_sst_colormap(target[0], ocean_mask[0])
                    
                    writer.add_image("sst/prediction", pred_colored, global_step)
                    writer.add_image("sst/target", target_colored, global_step)
